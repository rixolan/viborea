...
- `PLAYER_COOKIE_SECRET` is **required**: compose refuses to start without it and the app throws at boot. Generate with `openssl rand -base64 32`, store it in the Dokploy env UI (not only `.env`, which Redeploy rewrites). Rotating: move the old value to `PLAYER_COOKIE_SECRET_OLD` so manage links already sent keep working.

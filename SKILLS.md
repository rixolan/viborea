...
5. Área del jugador: `/reservar/:slug/clases`. Entrar jugador `/entrar/jugador/:slug` (no activa org). Entrar academia `/entrar/academia`. No revivir `/jugador` as a third shell. Confirmación: signed-in player with Clerk name + phone (required at signup) sees those data and “Editar en tu cuenta”; do not re-ask. Guest still fills name + WhatsApp.

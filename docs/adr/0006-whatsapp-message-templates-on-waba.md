...
Rejected: (a) Chatwoot as the authoring UI; (b) unofficial clients (wacli / WAHA / OpenWA); (c) treating Chatwoot canned replies as Meta message templates.

The production inbox cut (ADR 0007) does not move template authoring into Chatwoot. After the cut, Chatwoot may **send** `APPROVED` templates on the academy WABA; it still must not create them.
